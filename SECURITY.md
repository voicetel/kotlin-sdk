...
<https://github.com/voicetel/kotlin-sdk/security/advisories/new>).
...
- The SDK builds on Ktor's CIO engine with the JVM's default TLS
  posture (TLS 1.2+, session resumption via the JSSE session cache)
  and Ktor's `ContentEncoding` plugin for gzip. If you supply a
  custom `HttpClientEngine`, you are responsible for matching that
  posture.
...
Out of scope: vulnerabilities in the `com.voicetel:voicetel-sdk-kotlin`
